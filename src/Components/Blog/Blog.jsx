...
const Blog = () => {
    return (
        <div className='my-container'>
            ...
            <p>useMemo is a React hook that can be used to maximize speed by memoizing a value, which means that the value is only recomputed when the dependencies change.It accepts a function and an array of dependencies as parameters and returns a memoized value.UseMemo returns the value that was previously computed if the dependencies have not changed.When a component renders frequently yet has an expensive calculation or when a computationally expensive function is regularly used, it can be helpful.we can reduce the need for pointless re-renders and enhance efficiency by memoizing the value.
